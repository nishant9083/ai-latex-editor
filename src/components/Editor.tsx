...
export default function CodeEditor() {
  ...
  // Show "Saved" indicator when latexCode changes
  useEffect(() => {
    if (latexCode) {
      setShowSaved(true);
  ...
  const handleManualCompile = async () => {
    ...
    try {
      const result = await LaTeXCompiler.compile(latexCode, compilationAbortController.current.signal);
  ...
  return (
    <div className="h-full flex flex-col bg-gray-900">
      ...
      <div className="px-4 py-2 bg-gray-800 border-b border-gray-700">
        ...
        {/* Error Display */}
        {compilationError && (
          <div className="mt-2 px-3 py-2 bg-red-900/30 border border-red-700/50 rounded text-xs text-red-300 flex items-start gap-2">
